...
function tmux_wrapper_apply_bindings()
{
    ...
    if tmux_wrapper_is_enabled "$tmux_conf_new_pane_reconnect_ssh"; then
        if tmux_wrapper_is_enabled "$tmux_conf_new_pane_retain_current_path"; then
            tmux  unbind '"'
            tmux  unbind %
            tmux  bind -    run "cat ~/.tmux.conf.sh | sh -s tmux_wrapper_split_window #{pane_tty} -v -c '#{pane_current_path}'" \;\
                  bind \    run "cat ~/.tmux.conf.sh | sh -s tmux_wrapper_split_window #{pane_tty} -h -c '#{pane_current_path}'"
        else
            tmux  unbind '"'
            tmux  unbind %
            tmux  bind -    run "cat ~/.tmux.conf.sh | sh -s tmux_wrapper_split_window #{pane_tty} -v" \;\
                  bind \    run "cat ~/.tmux.conf.sh | sh -s tmux_wrapper_split_window #{pane_tty} -h"
        fi
    else
        if tmux_wrapper_is_enabled "$tmux_conf_new_pane_retain_current_path"; then
            tmux  unbind '"'
    ...
    if tmux -q -L swap-pane-test -f /dev/null new-session -d \; new-window \; new-window \; swap-pane -t :1 \; kill-session; then
        ...
        #
        # tmux bind + run 'cat ~/.tmux.conf.sh | sh -s tmux_wrapper_maximize_pane #{session_name} #D'
...
tmux_wrapper_apply_theme()
{
    ...
    tmux_conf_theme_window_status_format=$(echo "$tmux_conf_theme_window_status_format" | \
                                               sed 's%#{circled_window_index}%#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_circled_digit #I)%g')
    ...
    tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | \
                                      sed -e "s%#{root}%#[fg=$tmux_conf_theme_root_fg]#[bg=$tmux_conf_theme_root_bg]"`
                                           `"#[$tmux_conf_theme_root_attr]#(cat ~/.tmux.conf.sh | "`
...
EOF
    ...
    case "$status_left $status_right" in
        *'#{battery_status}'*|*'#{battery_bar}'*|*'#{battery_hbar}'*|*'#{battery_vbar}'*|*'#{battery_percentage}'*)
            ...
            status_right="#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_battery)$status_right"
            ;;
    esac
    ...
    case "$status_left $status_right" in
        *'#{username}'*|*'#{hostname}'*|*'#{username_ssh}'*|*'#{hostname_ssh}'*)
            status_left=$(echo "$status_left" | \
                              sed -e 's%#{username}%#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_username #{pane_tty} false #D)%g' \
                                  -e 's%#{hostname}%#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_hostname #{pane_tty} false #D)%g' \
                                  -e 's%#{username_ssh}%#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_username #{pane_tty} true #D)%g' \
                                  -e 's%#{hostname_ssh}%#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_hostname #{pane_tty} true #D)%g')
    ...
    case "$status_left $status_right" in
        *'#{uptime_d}'*|*'#{uptime_h}'*|*'#{uptime_m}'*)
            ...
            status_right=$(echo "$status_right" | sed -E \
                                                      ...
                                                      -e 's/#\{(\?)?uptime_m/#\{\1@uptime_m/g' \
                                                      -e 's/#\{(\?)?uptime_s/#\{\1@uptime_s/g')
            status_right="#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_uptime)$status_right"
            ;;
    esac
    ...
    case "$status_left $status_right" in
        *'#{loadavg}'*)
            ...
            status_right=$(echo "$status_right" | sed -E \
                                                      -e 's/#\{(\?)?loadavg/#\{\1@loadavg/g')
            status_right="#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_loadavg)$status_right"
            ;;
    esac

    # Style session number information
    status_left=$(echo "$status_left" | sed 's%#{circled_session_name}%#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_circled_digit #S)%g')
    status_right=$(echo "$status_right" | sed 's%#{circled_session_name}%#(cat ~/.tmux.conf.sh | sh -s tmux_wrapper_circled_digit #S)%g')
