...
@test "completion bash-it: disable - provide the python aliases when python plugin is enabled with the old location and name" {
  ...
  ln -s $BASH_IT/plugins/available/python.plugin.bash $BASH_IT/plugins/enabled/python.plugin.bash
  assert_link_exist "$BASH_IT/plugins/enabled/python.plugin.bash"

  run __check_completion 'bash-it disable alias py'
  assert_line -n 0 "python"
}

@test "completion bash-it: disable - provide the python aliases when python plugin is enabled with the old location and priority-based name" {
  ...
  ln -s $BASH_IT/plugins/available/python.plugin.bash $BASH_IT/plugins/enabled/350---python.plugin.bash
  assert_link_exist "$BASH_IT/plugins/enabled/350---python.plugin.bash"

  run __check_completion 'bash-it disable alias py'
  assert_line -n 0 "python"
}

@test "completion bash-it: disable - provide the python aliases when python plugin is enabled with the new location and priority-based name" {
  ln -s $BASH_IT/aliases/available/python.aliases.bash $BASH_IT/enabled/150---python.aliases.bash
  assert_link_exist "$BASH_IT/enabled/150---python.aliases.bash"

  ln -s $BASH_IT/plugins/available/python.plugin.bash $BASH_IT/enabled/350---python.plugin.bash
  assert_link_exist "$BASH_IT/enabled/350---python.plugin.bash"

  run __check_completion 'bash-it disable alias py'
  assert_line -n 0 "python"
}

@test "completion bash-it: enable - provide the ansible aliases when not enabled" {
  run __check_completion 'bash-it enable alias ans'
  assert_line -n 0 "ansible"
}

@test "completion bash-it: enable - provide the a* aliases when not enabled" {
  run __check_completion 'bash-it enable alias a'
  assert_line -n 0 "all ag ansible"
}
