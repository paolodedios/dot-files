...
function describe_ecr_repository()
{
    ...
    if [ $? -ne 0 ]; then
        error "Unable to get repository info for '${1}' from registry with id '$AWS_ACCOUNT_NUMBER'"
        return 1
    fi

    if [[ $repository_info == *"AccessDeniedException"* ]]; then
        error "Unable to get repository info for '${1}' from registry with id '$AWS_ACCOUNT_NUMBER'. Access Denied"
        return 1
    fi

    echo "$repository_info" | jq .
