#!/bin/bash
#
# tmux command wrapper utilities used for comaptibility between various
# versions of tmux
#
...
#
# Implementation note:
# String literal line continuations are implemented by using backtick operators
# that evaluate spaces to nothing
#
# @see https://stackoverflow.com/questions/7729023/how-do-i-break-up-an-extremely-long-string-literal-in-bash
#
###############################################################################
...
###############################################################################
#
# New line variable used with IFS
#
###############################################################################
__newline='
'
...
###############################################################################
#
# Helper function for using 'urlview' app from tmux
#
###############################################################################

function tmux_wrapper_urlview()
{
...
###############################################################################
#
# Helper function for using Facebook path picker 'fpp' app from tmux
#
###############################################################################

function tmux_wrapper_fpp()
{
...
function tmux_wrapper_is_enabled()
{
...
###############################################################################
#
# Provide for circle styled digit
#
###############################################################################

function tmux_wrapper_circled_digit()
{
    circled_digits='⓪ ① ② ③ ④ ⑤ ⑥ ⑦ ⑧ ⑨ ⑩ ⑪ ⑫ ⑬ ⑭ ⑮ ⑯ ⑰ ⑱ ⑲ ⑳'
...
###############################################################################
#
# Maximize curent pane
#
###############################################################################

function tmux_wrapper_maximize_pane()
{
    current_session=${1:-$(tmux display -p '#{session_name}')}
    current_pane=${2:-$(tmux display -p '#{pane_id}')}

    dead_panes=$(tmux list-panes -s -t "$current_session" \
                      -F '#{pane_dead} #{pane_id} #{pane_start_command}' |\ grep -o '^1 %.\+maximized.\+$' || true)
    restore=$(echo "$dead_panes" | \
                  sed -n -E \
                      -e "s/^1 $current_pane .+maximized.+(%[0-9]+)$/tmux swap-pane -s"`
                          `" \1 -t $current_pane \; kill-pane -t $current_pane/p" \
                      -e "s/^1 (%[0-9]+) .+maximized.+$current_pane$/tmux swap-pane -s \1 -t $current_pane \; kill-pane -t \1/p" )

    if [ -z "$restore" ]; then
        [ "$(tmux list-panes -t "$current_session:" | wc -l | sed 's/^ *//g')" -eq 1 ] && \
            tmux display "Can't maximize with only one pane" && return

        window=$(tmux new-window -t "$current_session:" \
                      -P "exec maximized... 2> /dev/null & tmux setw -t $current_session: remain-on-exit on;"`
                               `" printf 'Pane has been maximized, press <prefix>+ to restore. %s' \\$current_pane")
        window=${window%.*}

        guard=10
        while [ x"$(tmux list-panes -t "$window" -F
        '#{session_name}:#{window_index} #{pane_dead}' 2>/dev/null)" != x"$window 1" ] && \
                  [ "$guard" -ne 0 ]; do
            sleep 0.01
            guard=$((guard - 1))
        done
...
function tmux_wrapper_toggle_mouse()
{
    if tmux show -g -w | grep -q mode-mouse; then
        # Set mouse option for tmux < 2.4
...
function tmux_wrapper_battery()
{
    ...
    if echo "$battery_bar_palette" | grep -q -E '^heat|gradient(,[#a-z0-9]{7,9})?$'; then
        ...
        # shellcheck disable=SC2046
        [ "$empty" -gt 0 ] && \
            battery_bar="$battery_bar$(printf "#[fg=colour%s]$battery_bar_symbol_empty""`
                         `" $(echo "$palette" | cut -d' ' -f$((full + 1))-$((full + empty))))"
...
function tmux_wrapper_username()
{
    ...
    if [ -n "$ssh_parameters" ]; then
        ...
        # shellcheck disable=SC2086
        [ -z "$username" ] && username=$(ssh -T -o ControlPath=none \
                                             -o ProxyCommand="sh -c 'echo %%username%% %r >&2'" \
                                         $ssh_parameters 2>&1 | awk '/^%username% / { print $2; exit }')
    else
        if ! tmux_wrapper_is_enabled "$ssh_only"; then
            # shellcheck disable=SC2039
            if [ x"$OSTYPE" = x"cygwin" ]; then
                username=$(whoami)
            else
...
function tmux_wrapper_hostname()
{
    tty=${1:-$(tmux display -p '#{pane_tty}')}
    ssh_only=$2
    # shellcheck disable=SC2039
    ...
    if [ -n "$ssh_parameters" ]; then
        ...
        # shellcheck disable=SC2086
        [ -z "$hostname" ] && hostname=$(ssh -T -o ControlPath=none \
                                             -o ProxyCommand= "sh -c 'echo %%hostname%% %h >&2'" \
...
###############################################################################
#
# Provide root login indicator
#
###############################################################################

function tmux_wrapper_root()
{
    tty=${1:-$(tmux display -p '#{pane_tty}')}
    username=$(tmux_wrapper_username "$tty" false)
...
###############################################################################
#
# Provide system uptime information
#
###############################################################################

function tmux_wrapper_uptime()
{
    case $(uname -s) in
        *Darwin*)
...
###############################################################################
#
# Provide system load average information
#
###############################################################################

function tmux_wrapper_loadavg()
{
    case $(uname -s) in
        *Darwin*)
...
###############################################################################
#
# Split window override function
#
###############################################################################

function tmux_wrapper_split_window()
{
    tty=${1:-$(tmux display -p '#{pane_tty}')}
    shift
    # shellcheck disable=SC2039
...
###############################################################################
#
# Apply terminal override settings
#
###############################################################################

function tmux_wrapper_apply_overrides()
{
    tmux_conf_theme_24b_colour=${tmux_conf_theme_24b_colour:-false}
    if tmux_wrapper_is_enabled "$tmux_conf_theme_24b_colour"; then
        case "$TERM" in
            screen-*|tmux-*)
            ;;
            *)
...
###############################################################################
#
# Apply key-binding preference overrides
#
###############################################################################

function tmux_wrapper_apply_bindings()
{
    tmux_conf_new_window_retain_current_path=${tmux_conf_new_window_retain_current_path:-false}
    if tmux_wrapper_is_enabled "$tmux_conf_new_window_retain_current_path"; then
    ...
    if tmux_wrapper_is_enabled "$tmux_conf_new_pane_reconnect_ssh"; then
        if tmux_wrapper_is_enabled "$tmux_conf_new_pane_retain_current_path"; then
            tmux  unbind '"'
            tmux  unbind %
            tmux  bind -    run "cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_split_window #{pane_tty} -v -c '#{pane_current_path}'" \;\
                  bind \    run "cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_split_window #{pane_tty} -h -c '#{pane_current_path}'"
        else
            tmux  unbind '"'
            tmux  unbind %
            tmux  bind -    run "cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_split_window #{pane_tty} -v" \;\
                  bind \    run "cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_split_window #{pane_tty} -h"
        fi
    else
        if tmux_wrapper_is_enabled "$tmux_conf_new_pane_retain_current_path"; then
            tmux  unbind '"'
    ...
    if tmux_wrapper_is_enabled "$tmux_conf_new_session_prompt"; then
    ...
    if tmux -q -L swap-pane-test -f /dev/null new-session -d \; new-window \; new-window \; swap-pane -t :1 \; kill-session; then
        # Use the built-in 'resize-pane' option for pane zooming instead of the
        # custom implementation tmux_wrapper_maximize_pane
        #
        # tmux bind + run 'cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_maximize_pane #{session_name} #D'
    ...
    if tmux_wrapper_is_enabled "$tmux_conf_copy_to_os_clipboard"; then
...
###############################################################################
#
# Apply theme settings from .tmux.conf.local
#
###############################################################################

tmux_wrapper_apply_theme()
{
    #
    # Set window pane settings
    #
    ...
    if tmux show -g -w | grep -q window-style; then
        ...
        if tmux_wrapper_is_enabled "$tmux_conf_theme_highlight_focused_pane"; then
    ...
    tmux setw -g pane-border-style "fg=$tmux_conf_theme_pane_border_fg,bg=$tmux_conf_theme_pane_border_bg" \;\
    ...
    tmux set -g display-panes-colour "$tmux_conf_theme_pane_indicator" \;\
    ...
    tmux set -g message-command-style "fg=$tmux_conf_theme_message_command_fg,"`
                                       `"bg=$tmux_conf_theme_message_command_bg,$tmux_conf_theme_message_command_attr"
    ...
    if [ x"$(tmux show -g -v status-justify)" = x"right" ]; then
        tmux_conf_theme_window_status_current_format="#[fg=$tmux_conf_theme_window_status_current_bg,"`
                                     `"bg=$tmux_conf_theme_window_status_bg]$tmux_conf_theme_right_separator_main"`
                                     `"#[fg=default,bg=default,default]$tmux_conf_theme_window_status_current_format"`
                                     `" #[fg=$tmux_conf_theme_window_status_bg,bg=$tmux_conf_theme_window_status_current_bg,none]"`
                                     `"$tmux_conf_theme_right_separator_main"
    else
        tmux_conf_theme_window_status_current_format="#[fg=$tmux_conf_theme_window_status_bg,"`
                                `"bg=$tmux_conf_theme_window_status_current_bg]$tmux_conf_theme_left_separator_main"`
                                `"#[fg=default,bg=default,default]$tmux_conf_theme_window_status_current_format"`
                                `" #[fg=$tmux_conf_theme_window_status_current_bg,bg=$tmux_conf_theme_status_bg,none]"`
                                `"$tmux_conf_theme_left_separator_main"
    fi

    tmux_conf_theme_window_status_format=$(echo "$tmux_conf_theme_window_status_format" | \
                                               sed 's%#{circled_window_index}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_circled_digit #I)%g')
    tmux_conf_theme_window_status_current_format=$(echo "$tmux_conf_theme_window_status_current_format" | \
                                               sed 's%#{circled_window_index}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_circled_digit #I)%g')

    tmux  setw -g window-status-style "fg=$tmux_conf_theme_window_status_fg,"`
                                        `"bg=$tmux_conf_theme_window_status_bg,$tmux_conf_theme_window_status_attr" \;\
          setw -g window-status-format "$tmux_conf_theme_window_status_format" \;\
          setw -g window-status-current-style "fg=$tmux_conf_theme_window_status_current_fg,"`
                                               `"bg=$tmux_conf_theme_window_status_current_bg,$tmux_conf_theme_window_status_current_attr" \;\
    ...
    tmux setw -g window-status-activity-style "fg=$tmux_conf_theme_window_status_activity_fg,"`
                                                `"bg=$tmux_conf_theme_window_status_activity_bg,$tmux_conf_theme_window_status_activity_attr"
    ...
    tmux setw -g window-status-bell-style "fg=$tmux_conf_theme_window_status_bell_fg,"`
    ...
    tmux setw -g window-status-last-style "fg=$tmux_conf_theme_window_status_last_fg,"`
                                            `"bg=$tmux_conf_theme_window_status_last_bg,$tmux_conf_theme_window_status_last_attr"
    #
    # Set up indicators
    #
    ...
    tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | \
                                      sed -e "s/#{pairing}/#[fg=$tmux_conf_theme_pairing_fg]#[bg=$tmux_conf_theme_pairing_bg]"`
                                           `"#[$tmux_conf_theme_pairing_attr]#{?session_many_attached,$tmux_conf_theme_pairing,}/g")

    tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | \
                                      sed -e "s/#{prefix}/#[fg=$tmux_conf_theme_prefix_fg]#[bg=$tmux_conf_theme_prefix_bg]"`
                                           `"#[$tmux_conf_theme_prefix_attr]#{?client_prefix,$tmux_conf_theme_prefix,}/g")

    tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | \
                                      sed -e "s%#{root}%#[fg=$tmux_conf_theme_root_fg]#[bg=$tmux_conf_theme_root_bg]"`
                                           `"#[$tmux_conf_theme_root_attr]#(cat ${TMUX_THEME_FILE} | "`
                                           `"sh -s tmux_wrapper_root #{pane_tty} #D)#[inherit]%g")
...
EOF
        ...
        #
        # Check if we are running a tmux in between v1.9 and v2.0
        if [ x"$(tmux -q -L tmux_theme_status_left_test -f /dev/null new-session -d \; show -g -v status-left \; kill-session)" = x"[#S] " ];
        then
           case "$status_left" in
                *\ )
                ;;
    ...
    #
    # Set right status line style
    #
    tmux_conf_theme_status_right=${tmux_conf_theme_status_right-'#{pairing}#{prefix} #{battery_status} #{battery_bar} #{battery_percentage} '`
                                                                                     `', %R , %d %b | #{username} | #{hostname} '}
    ...
    tmux_conf_theme_status_right=$(echo "$tmux_conf_theme_status_right" | \
                                       sed -e "s/#{pairing}/#[fg=$tmux_conf_theme_pairing_fg]#[bg=$tmux_conf_theme_pairing_bg]"`
                                            `"#[$tmux_conf_theme_pairing_attr]#{?session_many_attached,$tmux_conf_theme_pairing,}/g")
    ...
    tmux_conf_theme_status_right=$(echo "$tmux_conf_theme_status_right" | \
                                       sed -e "s%#{root}%#[fg=$tmux_conf_theme_root_fg]#[bg=$tmux_conf_theme_root_bg]"`
                                            `"#[$tmux_conf_theme_root_attr]#(cat ${TMUX_THEME_FILE} | "`
                                            `"sh -s tmux_wrapper__root #{pane_tty} #D)#[inherit]%g")
...
EOF
    ...
    #
    # Enable user specified variables
    #
    ...
    case "$status_left $status_right" in
        *'#{battery_status}'*|*'#{battery_bar}'*|*'#{battery_hbar}'*|*'#{battery_vbar}'*|*'#{battery_percentage}'*)
            ...
            status_right="#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_battery)$status_right"
            ;;
    esac
    ...
    case "$status_left $status_right" in
        *'#{username}'*|*'#{hostname}'*|*'#{username_ssh}'*|*'#{hostname_ssh}'*)
            status_left=$(echo "$status_left" | \
                              sed -e 's%#{username}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_username #{pane_tty} false #D)%g' \
                                  -e 's%#{hostname}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_hostname #{pane_tty} false #D)%g' \
                                  -e 's%#{username_ssh}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_username #{pane_tty} true #D)%g' \
                                  -e 's%#{hostname_ssh}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_hostname #{pane_tty} true #D)%g')
            status_right=$(echo "$status_right" | \
                              sed -e 's%#{username}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_username #{pane_tty} false #D)%g' \
                                  -e 's%#{hostname}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_hostname #{pane_tty} false #D)%g' \
                                  -e 's%#{username_ssh}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_username #{pane_tty} true #D)%g' \
                                  -e 's%#{hostname_ssh}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_hostname #{pane_tty} true #D)%g')
    ...
    case "$status_left $status_right" in
        *'#{uptime_d}'*|*'#{uptime_h}'*|*'#{uptime_m}'*)
            ...
            status_right=$(echo "$status_right" | sed -E \
                                                      ...
                                                      -e 's/#\{(\?)?uptime_m/#\{\1@uptime_m/g' \
                                                      -e 's/#\{(\?)?uptime_s/#\{\1@uptime_s/g')
            status_right="#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_uptime)$status_right"
            ;;
    esac
    ...
    case "$status_left $status_right" in
        *'#{loadavg}'*)
            ...
            status_right=$(echo "$status_right" | sed -E \
                                                      -e 's/#\{(\?)?loadavg/#\{\1@loadavg/g')
            status_right="#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_loadavg)$status_right"
            ;;
    esac

    # Style session number information
    status_left=$(echo "$status_left" | sed 's%#{circled_session_name}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_circled_digit #S)%g')
    status_right=$(echo "$status_right" | sed 's%#{circled_session_name}%#(cat ${TMUX_THEME_FILE} | sh -s tmux_wrapper_circled_digit #S)%g')
...
###############################################################################
#
# Apply configuration settings in .tmux.conf.local
# Settings from .tmux.conf.local should be sourced ahead of time
#
###############################################################################

function tmux_wrapper_apply_configuration()
{
    ...
    tmux_wrapper_apply_overrides
    tmux_wrapper_apply_bindings
    tmux_wrapper_apply_theme
