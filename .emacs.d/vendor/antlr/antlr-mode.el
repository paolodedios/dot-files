...
This variable might be set depending on `antlr-tool-version'.")
...
;; on Emacs-25.1.1), try M-x imenu RET -> you see all.
...
(defun antlr-insert-keyword-rule (&optional keyword)
  ...
  (insert (upcase keyword) " : "
          (mapconcat (lambda (c) (string ?\[ (downcase c) (upcase c) ?\]))
                     keyword "")
          " ;\n"))
...
(define-derived-mode antlr-mode prog-mode
  ...
  ;; The menu is broken in Emacs-25.1.1, but completion with M-x imenu is fine
